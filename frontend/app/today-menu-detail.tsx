...
import { View, Text, SafeAreaView, StatusBar, ScrollView, TouchableOpacity, Image, Dimensions } from 'react-native';
...
export default function TodayMenuDetail() {
  ...
  const combinedAllergies = useMemo(() => {
    ...
    allergies.forEach((a: string) => { if (a) set.add(a); });
    relatedMenus.forEach((m: any) => {
      ...
      arr.forEach((a: string) => { if (a) set.add(a); });
    });
  ...
  useEffect(() => {
    ...
    const fetchFood = async () => {
      if (!menuId) return;
      try {
        const url = `http://localhost:8082/api/menus/${menuId}/food`;
  ...
  useEffect(() => {
    ...
    const fetchRestaurantMenus = async () => {
      if (!resNo) return;
      try {
        const url = `http://localhost:8082/api/menus/restaurant/${resNo}`;
        const res = await fetch(url);
        if (!res.ok) return;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent}>
        ...
        {/* 메인 이미지 */}
        <View style={{ width: '100%', height: width * 0.6, borderRadius: 16, overflow: 'hidden', backgroundColor: '#000', marginBottom: 16 }}>
          {mainDish?.image && (
            <Image source={mainDish.image} style={{ width: '100%', height: '100%', resizeMode: 'cover' }} />
          )}
        </View>
          ...
          <View style={styles.sideDishItem}>
            <View style={styles.dishInfo}>
              <View style={styles.dishNameContainer}>
                <Text style={styles.dishName}>{foodDetail?.menuName || foodDetail?.menu_name || mainDish?.name}</Text>
                ...
                {soldout && <Text style={[styles.dishName, { color: '#E53935' }]}> (품절)</Text>}
              </View>
              {!!(foodDetail?.kcal || mainDish?.calories) && <Text style={styles.dishCalories}>{(foodDetail?.kcal || mainDish?.calories)} Kcal</Text>}
            </View>
          </View>
        </View>
        ...
        <View style={styles.sideDishesSection}>
          ...
          {relatedMenus.map((m: any, index: number) => {
            const allergies = m?.food?.allergyInfo
            ...
            return (
              <View key={index} style={styles.sideDishItem}>
                <View style={styles.dishInfo}>
                  <View style={styles.dishNameContainer}>
                    <Text style={styles.dishName}>{m?.food?.menuName || m?.food?.menu_name || m?.food?.content}</Text>
                  </View>
                  {!!m?.food?.kcal && (
          ...
          <View style={styles.nutritionCard}>
            ...
            <View style={styles.calorieProgressBar}>
              <View style={styles.progressBarBackground}>
                <View style={[styles.progressBarFill, { width: `${Math.min((totalCalories / 2000) * 100, 100)}%` }]} />
              </View>
              <Text style={styles.progressText}>일일 권장량의 {Math.round((totalCalories / 2000) * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 15 }}>
          <TouchableOpacity style={styles.actionButton} activeOpacity={0.7} onPress={() => router.push({
            pathname: '/payment',
            params: {
              title,
